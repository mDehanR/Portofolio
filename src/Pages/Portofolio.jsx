...
export default function FullWidthTabs() {
  ...
  const [certificates, setCertificates] = useState([{ id: 1, Img: "/images/certificate-1.svg" }]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [projectsResponse, certificatesResponse] = await Promise.all([supabase.from("projects").select("*").order("id", { ascending: false }), supabase.from("certificates").select("*").order("id", { ascending: false })]);
      ...
      const finalProjects = projectData.length > 0 ? projectData : sampleProjects;

      const finalCertificates = certificateData.length > 0 ? certificateData : [{ id: 1, Img: "/images/certificate-1.svg" }];
  ...
  useEffect(() => {
    ...
    if (cachedProjects && cachedCertificates) {
      ...
      setCertificates([{ id: 1, Img: "/images/certificate-1.svg" }]);
    }
